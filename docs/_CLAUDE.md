...
- **[Protocols](protocols/)** defines shared types and behavioral contracts. Types: Tool, ToolOutput, result types (FileContent, GrepResult, etc.), TaskSpec, TaskState, Execution variants, event dataclasses, Action types (ScriptAction, LogAction, WorkflowAction, EventAction), dispatch types (Subscription, FilterPredicate, Source), identity/auth types (Principal, AuthContext, TrustTier, ConsumerRecord), Capability (declaring its `required_scope` and `injects`). Contracts: EventDelivery, StorageBackend, OverseerProtocol, DispatchBackend, ActionExecutor, FlushScheduler, EventBus, SessionProtocol, Renderer, PrincipalStorage. Also provides `run_sync()` for event-loop-aware sync-to-async bridging.
