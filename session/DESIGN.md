...
| `_pricing.py` | Internal pricing table: per-model input/output/cache/reasoning token rates. `compute_cost_usd(model, usage) -> Decimal`. Updated by orxt's developers when provider prices change. Used at reporting time to compute best-effort USD from token counts. |
