...
class TraceWriter:
    ...
    async def create_run(
        ...
        *,
        ...
        The ``run_id`` is generated by the orchestrating layer *before* the
        run's principal is minted, so the row and its principal share the same
        id. ``created_by`` is the persisted principal of the actor that
        started the run.
        """
        async with self._pool.acquire() as conn, conn.transaction():
            await conn.execute(
                "INSERT INTO runs"
                " (id, intent, config_snapshot, autonomy_level, created_by)"
                " VALUES ($1, $2, $3, $4, $5)",
                run_id,
                intent,
                json.dumps(config),
