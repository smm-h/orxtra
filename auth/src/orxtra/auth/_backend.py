...
class AuthBackend:
    ...
    async def create_consumer(
        ...
        *,
        ...
        Both ids are generated by the caller *before* this call: the caller
        mints the consumer's own principal (kind=consumer,
        external_ref=consumer_id) first, so the principal exists before the
        row that FKs into it. Auth never imports identity -- the mint happens
        in the orchestrating layer and the resulting ids flow in here.
        """
        async with self._pool.acquire() as conn, conn.transaction():
            row = await conn.fetchrow(
                "INSERT INTO consumers"
                " (id, principal_id, name, trust_tier, scope_grants)"
                " VALUES ($1, $2, $3, $4, $5)"
                " RETURNING id",
                consumer_id,
                principal_id,
    ...
    async def get_consumer(
        ...
        async with self._pool.acquire() as conn, conn.transaction():
            row = await conn.fetchrow(
                "SELECT id, principal_id, name, trust_tier, scope_grants,"
